...
import type { Supplier, SupplierKPIs } from '@/types';
import type { StockItem, CompanyProfile } from '@/types';
...
function buildKPISummary(p: ReportParams): any[][] {
  ...
  return [
    ['KPI EXECUTIVE SUMMARY', '', p.companyProfile?.name || 'ProcureBuddy'],
...
export async function exportManagementReport(
  ...
  const metaSheet = XLSX.utils.aoa_to_sheet([
    ...
    ['Generated By',      'ProcureBuddy Platform'],
    ['Company',           params.companyProfile?.name || 'ProcureBuddy'],
  ...
  const fileName = `ProcureBuddy_${type.replace(/-/g, '_')}_${today}.xlsx`;
...
export async function exportFullReport(
  ...
  XLSX.writeFile(wb, `ProcureBuddy_Full_Report_${today}.xlsx`);
  onProgress?.('Done');
}
